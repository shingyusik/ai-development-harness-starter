...
핵심 시작점은 repo skill `.agents/skills/harness/SKILL.md`다. `/harness <task>` 또는 `$harness <task>`는 이 skill을 실행한다. `.harness/bootstrap.md`는 skill이 읽는 runtime contract다. 루트 `AGENTS.md`는 저장소 진입 지도 역할만 한다.
...
- `.agents/skills/harness/SKILL.md`: `/harness` 명령의 단일 entry skill과 하네스 행동 절차.
...
- 하네스 시작 명령과 행동 절차는 `.agents/skills/harness/SKILL.md` 하나에 둔다.
